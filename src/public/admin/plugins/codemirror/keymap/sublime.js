...
(function(mod) {
    ...
    function findPosSubword(doc, start, dir) {
        ...
        for (var pos = startPos, e = dir < 0 ? 0 : line.length, i = 0; pos != e; pos += dir, i++) {
            ...
            if (state == "start") {
                if (cat != "o") { state = "in";
                    type = cat; } else startPos = pos + dir
            } else if (state == "in") {
                if (type != cat) {
                    if (type == "w" && cat == "W" && dir < 0) pos--;
                    if (type == "W" && cat == "w" && dir > 0) { // From uppercase to lowercase
                        if (pos == startPos + 1) { type = "w"; continue; } else pos--;
                    }
                    break;
                }
            }
        ...
        cm.operation(function() {
            ...
            for (var i = 0; i < ranges.length; i++) {
                ...
                if (range.empty()) { indices.push(i);
                    replacements.push(""); } else replacements.push(mod(cm.getRange(range.from(), range.to())));
            }
    ...
    cmds.deleteToSublimeMark = function(cm) {
        ...
        if (found) {
            var from = cm.getCursor(),
                to = found;
            if (CodeMirror.cmpPos(from, to) > 0) { var tmp = to;
                to = from;
                from = tmp; }
